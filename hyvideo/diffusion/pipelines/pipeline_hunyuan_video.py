...
class HunyuanVideoPipeline(DiffusionPipeline):
    ...
    documentation for the generic methods
    implemented for all pipelines (downloading, saving, running on a
    ...
    Args:
        ...
        transformer ([`HYVideoDiffusionTransformer`]):
            A `HYVideoDiffusionTransformer` to denoise the encoded video
            latents.
        scheduler ([`SchedulerMixin`]):
            A scheduler to be used in combination with `unet` to denoise the
    ...
    def __init__(
        ...
        if (
            ...
            deprecation_message = (
                f"The configuration file of this scheduler: {scheduler}"
                f" is outdated. `steps_offset`"
                f" should be set to 1 instead of "
            ...
            deprecate(
                "steps_offset!=1", "1.0.0", deprecation_message,
                standard_warn=False
            )
        ...
        if (
            ...
            deprecation_message = (
                ...
                f"not set the configuration `clip_sample`."
                " `clip_sample` should be set to False in the configuration "
                "file. Please make sure to update the"
                " config accordingly as not setting `clip_sample` in the "
                "config might lead to incorrect results in"
                " future versions. If you have downloaded this checkpoint "
                "from the Hugging Face Hub, it would be very"
                " nice if you could open a Pull request for the "
                "`scheduler/scheduler_config.json` file"
            )
            deprecate(
                "clip_sample not set", "1.0.0", deprecation_message,
                standard_warn=False
            )
            new_config = dict(scheduler.config)
    ...
    def encode_prompt(
        ...
        Args:
            ...
            negative_prompt (`str` or `List[str]`, *optional*):
                The prompt or prompts not to guide the video generation. If
            ...
            clip_skip (`int`, *optional*):
                Number of layers to be skipped from CLIP while computing the
                prompt embeddings. A value of 1 means that
                the output of the pre-final layer will be used for computing
        ...
        if prompt_embeds is None:
            ...
            if clip_skip is None:
                ...
                prompt_embeds = prompt_outputs.hidden_states_list[
                    -(clip_skip + 1)]
                # We also need to apply the final LayerNorm here to not mess
                # with the
                # representations. The `last_hidden_states` that we typically
                # use for
                # obtaining the final prompt representations passes through
                # the LayerNorm
                # layer.
        ...
        if do_classifier_free_guidance and negative_prompt_embeds is None:
            ...
            if semantic_images is not None:
                uncond_image = [black_image(img.size[0], img.size[1]) for img in
                                semantic_images]
            else:
                uncond_image = None

            negative_prompt_outputs = text_encoder.encode(
                uncond_input, data_type=data_type, semantic_images=uncond_image,
                device=device
            )
    ...
    def decode_latents(self, latents, enable_tiling=True):
        deprecation_message = ("The decode_latents method is deprecated and "
                               "will be removed in 1.0.0. Please use "
                               "VaeImageProcessor.postprocess(...) instead")
        deprecate("decode_latents", "1.0.0", deprecation_message,
                  standard_warn=False)
    ...
    def check_inputs(
        ...
        if video_length is not None:
            if "884" in vae_ver:
                if video_length != 1 and (video_length - 1) % 4 != 0:
                    raise ValueError(
                        f"`video_length` has to be 1 or a multiple of 4 but "
                        f"is {video_length}."
                    )
            elif "888" in vae_ver:
                if video_length != 1 and (video_length - 1) % 8 != 0:
                    raise ValueError(
                        f"`video_length` has to be 1 or a multiple of 8 but "
                        f"is {video_length}."
                    )
        ...
        if callback_on_step_end_tensor_inputs is not None and not all(
            ...
            raise ValueError(
                ...
                f"{[k for k in callback_on_step_end_tensor_inputs \
        ...
        if prompt is not None and prompt_embeds is not None:
            raise ValueError(
                ...
                f"{prompt_embeds}. Please make sure to"
            ...
            raise ValueError(
                f"`prompt` has to be of type `str` or `list` but is "
                f"{type(prompt)}"
            )
    ...
    def prepare_latents(
        ...
        if i2v_mode:
            ...
            x0 = randn_tensor(shape, generator=generator, device=device,
                              dtype=dtype)
            x1 = img_latents
        ...
        # Check existence to make it compatible with
    ...
    def get_guidance_scale_embedding(
        ...
        Args:
            w (`torch.Tensor`):
                Generate embedding vectors with a specified guidance scale to
                subsequently enrich timestep embeddings.
        ...
        Returns:
            `torch.Tensor`: Embedding vectors with shape `(len(w),
            embedding_dim)`.
        """
    ...
    def __call__(
        ...
        Args:
            ...
            sigmas (`List[float]`, *optional*):
                Custom sigmas to use for the denoising process with
                schedulers which support a `sigmas` argument in
                their `set_timesteps` method. If not defined, the default
                behavior when `num_inference_steps` is passed
                will be used.
            ...
            eta (`float`, *optional*, defaults to 0.0):
                ...
                to the [`~schedulers.DDIMScheduler`], and is ignored in other
                schedulers.
            generator (`torch.Generator` or `List[torch.Generator]`,
            *optional*):
                A [`torch.Generator`](
                https://pytorch.org/docs/stable/generated/torch.Generator
                .html) to make
            ...
            prompt_embeds (`torch.Tensor`, *optional*):
                Pre-generated text embeddings. Can be used to easily tweak
                text inputs (prompt weighting). If not
                provided, text embeddings are generated from the `prompt`
                input argument.
            negative_prompt_embeds (`torch.Tensor`, *optional*):
                Pre-generated negative text embeddings. Can be used to easily
                tweak text inputs (prompt weighting). If
                not provided, `negative_prompt_embeds` are generated from the
            ...
            clip_skip (`int`, *optional*):
                ...
                the output of the pre-final layer will be used for computing
                the prompt embeddings.
            callback_on_step_end (`Callable`, `PipelineCallback`,
            `MultiPipelineCallbacks`, *optional*):
                A function or a subclass of `PipelineCallback` or
                `MultiPipelineCallbacks` that is called at the end of
                each denoising step during the inference. with the following
                arguments: `callback_on_step_end(self:
                DiffusionPipeline, step: int, timestep: int, callback_kwargs:
                Dict)`. `callback_kwargs` will include a
        ...
        Returns:
            [`~HunyuanVideoPipelineOutput`] or `tuple`:
                If `return_dict` is `True`, [`HunyuanVideoPipelineOutput`] is
                returned,
        ...
        if callback_steps is not None:
            deprecate(
                ...
                "is deprecated, consider using `callback_on_step_end`",
            )
        ...
        device = torch.device(
            f"cuda:{dist.get_rank()}") if dist.is_initialized() else (
        ...
        if i2v_mode:
            if img_latents.shape[2] == 1:
                img_latents_concat = img_latents.repeat(1, 1, video_length, 1,
                                                        1)
            else:
            ...
            mask_concat = torch.ones(img_latents_concat.shape[0], 1,
                                     ...
                                     img_latents_concat.shape[4]).to(
                device=img_latents.device)
            mask_concat[:, :, 1:, ...] = 0

        # 6. Prepare extra step kwargs.
        # OriginalTODO: Logic should ideally just be
            ...
            for i, t in enumerate(timesteps):
                ...
                with torch.autocast(
                    ...
                    noise_pred = self.transformer(
                        # For an input image (129, 192, 336) (1, 256, 256)
        ...
        if not output_type == "latent":
            ...
            if len(latents.shape) == 4:
                ...
                raise ValueError(
                    f"Only support latents with shape (b, c, h, w) or (b, c, "
                    f"f, h, w), but got {latents.shape}."
                )
