...
def add_train_denoise_schedule_args(parser: argparse.ArgumentParser):
    ...
    group.add_argument("--flow-path-type", type=str, default="linear",
                       choices=FLOW_PATH_TYPE,
                       help="Path type for flow matching schedulers.")
    group.add_argument("--flow-predict-type", type=str, default="velocity",
                       choices=FLOW_PREDICT_TYPE,
                       help="Prediction type for flow matching schedulers.")
    group.add_argument("--flow-loss-weight", type=str, default=None,
    ...
    group.add_argument("--flow-train-eps", type=float, default=None,
                       help="Small epsilon for avoiding instability during "
                            "training.")
    group.add_argument("--flow-sample-eps", type=float, default=None,
                       help="Small epsilon for avoiding instability during "
                            "sampling.")
    group.add_argument("--flow-snr-type", type=str, default="lognorm",
...
def add_deepspeed_args(parser: argparse.ArgumentParser):
    ...
    group.add_argument("--zero-stage", type=int, default=0,
                       choices=[0, 1, 2, 3],
                       help="DeepSpeed ZeRO stage. 0: off, 1: offload "
                            "optimizer, 2: offload parameters, "
...
def add_data_args(parser: argparse.ArgumentParser):
    ...
    group.add_argument("--data-jsons-path", type=str, default=None,
                       help="Dataset path for training.")
    group.add_argument("--sample-n-frames", type=int, default=65,
                       help="How many frames to sample from a video. if using "
                            "3d vae, the number should be 4n+1")
    group.add_argument("--sample-stride", type=int, default=1,
                       help="How many frames to skip when sampling from a "
                            "video.")
    ...
    group.add_argument("--same-data-batch", action="store_true",
                       help="Use same data type for all rank in a batch for "
                            "training.")
    group.add_argument("--uncond-p", type=float, default=0.1,
                       help="Probability of randomly dropping video "
                            "description.")
    group.add_argument("--sematic-cond-drop-p", type=float, default=0.1,
                       help="Probability of randomly dropping img condition "
                            "description.")
...
def add_training_args(parser: argparse.ArgumentParser):
    ...
    group.add_argument("--global-batch-size", type=int, default=None, nargs='*',
                       help="Global batch size (across all model instances). "
                            ...
                            "world-size * gradient-accumulation-steps")
    ...
    group.add_argument("--resume", type=str, default=None,
                       help="Path to the checkpoint to resume training. It "
                            "can be an experiment index to resume from "
                            "the latest checkpoint in the output directory.")
    group.add_argument("--init-from", type=str, default=None,
                       help="Path to the checkpoint to load from init ckpt "
                            "for training. ")
    ...
    group.add_argument("--max-training-steps", type=int, default=10_000_000,
                       help="Maximum number of training steps.")
    group.add_argument("--ckpt-every", type=int, default=5000,
                       help="Save checkpoint every N steps.")

    group.add_argument("--rope-theta-rescale-factor", type=float, default=1.0,
                       nargs='+',
                       help="Rope interpolation factor.")
    group.add_argument("--rope-interpolation-factor", type=float, default=1.0,
                       nargs='+',
                       help="Rope interpolation factor.")
...
def add_optimizer_args(parser: argparse.ArgumentParser):
    ...
    group.add_argument("--lr", type=float, default=1e-4,
                       help="Basic learning rate, varies depending on "
                            "learning rate schedule and warmup.")
    group.add_argument("--warmup-min-lr", type=float, default=1e-6,
    ...
    group.add_argument("--adam-beta1", type=float, default=0.9,
                       help="[AdamW] First coefficient for computing running "
                            "averages of gradient.")
    group.add_argument("--adam-beta2", type=float, default=0.999,
                       help="[AdamW] Second coefficient for computing running "
                            "averages of gradient square.")
    group.add_argument("--adam-eps", type=float, default=1e-8,
                       help="[AdamW] Term added to the denominator to improve "
                            "numerical stability.")
...
def add_network_args(parser: argparse.ArgumentParser):
    ...
    group.add_argument(
        ...
        type=str,
        default=16,
        help="Number of latent channels of DiT. If None, it will be "
             "determined by `vae`. If provided, "
    ...
    group.add_argument(
        ...
        default="bf16",
        choices=PRECISIONS,
        help="Precision mode. Options: fp32, fp16, bf16. Applied to the "
             "backbone model and optimizer.",
    )
    ...
    group.add_argument("--gradient-checkpoint-layers", type=int, default=-1,
                       help="Number of layers to checkpoint. -1 for all "
                            "layers. `n` for the first n layers.")
...
def add_extra_models_args(parser: argparse.ArgumentParser):
    ...
    group.add_argument(
        "--apply-final-norm",
        action="store_true",
        help="Apply final normalization to the used text encoder hidden "
             "states.",
    )
...
def add_inference_args(parser: argparse.ArgumentParser):
    ...
    group.add_argument(
        ...
        type=str,
        default="ckpts",
        help="Root path of all the models, including t2v models and extra "
             "models.",
    )
    group.add_argument(
        "--dit-weight",
        type=str,
        default="ckpts/hunyuan-video-t2v-720p/transformers"
                "/mp_rank_00_model_states.pt",
        help="Path to the HunyuanVideo model. If None, search the model in "
             "the args.model_root."
             "1. If it is a file, load the model directly."
             "2. If it is a directory, search the model in the directory. "
    ...
    group.add_argument(
        "--i2v-dit-weight",
        type=str,
        default="ckpts/hunyuan-video-i2v-720p/transformers"
                "/mp_rank_00_model_states.pt",
        help="Path to the HunyuanVideo model. If None, search the model in "
             "the args.model_root."
             "1. If it is a file, load the model directly."
    ...
    group.add_argument(
        ...
        type=str,
        default="module",
        help="Key to load the model states. 'module' for the main model, "
             "'ema' for the EMA model.",
    )
    group.add_argument(
        "--use-cpu-offload",
        action="store_true",
    ...
    group.add_argument(
        "--disable-autocast",
        action="store_true",
        help="Disable autocast for denoising loop and vae decoding in "
             "pipeline sampling.",
    )
    group.add_argument(
        "--save-path",
    ...
    group.add_argument(
        ...
        nargs="+",
        default=(720, 1280),
        help="Video size for training. If a single value is provided, it will "
             "be used for both height "
             "and width. If two values are provided, they will be used for "
             "height and width "
             "respectively.",
    )
    group.add_argument(
        ...
        type=int,
        default=129,
        help="How many frames to sample from a video. if using 3d vae, "
             "the number should be 4n+1",
    )
    # --- prompt ---
    group.add_argument(
        "--prompt",
    ...
    group.add_argument(
        ...
        help="Seed type for evaluation. If file, use the seed from the CSV "
             "file. If random, generate a "
             "random seed. If fixed, use the fixed seed given by `--seed`. If "
             "auto, `csv` will use the "
             "seed column if available, otherwise use the fixed `seed` value. "
             "`prompt` will use the "
             "fixed `seed` value.",
    )
    ...
    group.add_argument(
        "--neg-prompt", type=str, default=None,
    ...
    group.add_argument(
        "--reproduce",
        action="store_true",
        help="Enable reproducibility by setting random seeds and "
...
def add_i2v_args(parser: argparse.ArgumentParser):
    ...
    group.add_argument(
        "--i2v-resolution", type=str, default="720p",
        choices=["720p", "540p", "360p"], help="Resolution for i2v inference."
    )

    group.add_argument(
        "--i2v-image-path", type=str, default="./assets/demo/i2v/imgs/0.png",
...
def add_lora_args(parser: argparse.ArgumentParser):
    ...
    group.add_argument(
        ...
        help="Fusion scale for lora model."
    )
...
def sanity_check_args(args):
    ...
    if not re.match(vae_pattern, args.vae):
        raise ValueError(
            f"Invalid VAE model: {args.vae}. Must be in the format"
            f" of '{vae_pattern}'."
        )
    ...
    if vae_channels != args.latent_channels:
        raise ValueError(
            f"Latent channels ({args.latent_channels}) must match the VAE "
            f"channels ({vae_channels})."
        )
    return args
