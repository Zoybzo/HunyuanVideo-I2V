...
class DecoderOutput(BaseOutput):
    ...
    Args:
        sample (`torch.FloatTensor` of shape `(batch_size, num_channels,
        height, width)`):
...
class EncoderCausal3D(nn.Module):
    r"""
    The `EncoderCausal3D` layer of a variational autoencoder that encodes its
    ...
    def __init__(
        ...
        for i, down_block_type in enumerate(down_block_types):
            ...
            if time_compression_ratio == 4:
                ...
                raise ValueError(
                    f"Unsupported time_compression_ratio: "
                    f"{time_compression_ratio}.")
...
class DecoderCausal3D(nn.Module):
    ...
    def __init__(
        ...
        self.conv_in = CausalConv3d(in_channels, block_out_channels[-1],
                                    kernel_size=3, stride=1)
        self.mid_block = None
        ...
        self.mid_block = UNetMidBlockCausal3D(
            ...
            resnet_act_fn=act_fn,
            output_scale_factor=1,
            resnet_time_scale_shift="default" if norm_type == "group" else
            norm_type,
            attention_head_dim=block_out_channels[-1],
            resnet_groups=norm_num_groups,
        ...
        for i, up_block_type in enumerate(up_block_types):
            ...
            upsample_scale_factor_HW = (2, 2) if add_spatial_upsample else (
            1, 1)
        ...
        if norm_type == "spatial":
            ...
            self.conv_norm_out = nn.GroupNorm(
                ...
                eps=1e-6)
        self.conv_act = nn.SiLU()
        self.conv_out = CausalConv3d(block_out_channels[0], out_channels,
                                     kernel_size=3)
...
class DiagonalGaussianDistribution(object):
    ...
    def sample(self, generator: Optional[
        torch.Generator] = None) -> torch.FloatTensor:
        # make sure sample is on the same device as the parameters and has
        # same dtype
        sample = randn_tensor(
            self.mean.shape,
            generator=generator,
